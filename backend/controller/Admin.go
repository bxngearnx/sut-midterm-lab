...
type CreateAdminPayload struct{
	...
	Admin_Password string   `json:"Admin_Password" valid:"minstringlength(8)"`
...
type  UpdateAdminPayload struct{
	ID uint `json:"ID"`
	Admin_Name     string	`json:"Admin_Name" valid:"required~name cannot be blank"`
	Admin_Email	   string   `json:"Admin_Email" valid:"email"`
...
func ListAdminByID(c *gin.Context) {
	...
	var Admin entity.ADMIN
	id := c.Param("id")
	if err := entity.DB().Raw("SELECT * FROM admins WHERE id = ?", id).First(&Admin).Error; err != nil {
