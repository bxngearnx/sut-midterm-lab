...
func CreatePostponement(c *gin.Context) {
	...
	//16:สร้าง entity POSTPONEMENT
		...
		// Postponement.StudentID =   Postponement.StudentID,
...
func ListPostponementTable(c *gin.Context) {
	...
	if err := entity.DB().Preload("Prefix").Preload("Degree").Preload("Trimester").Preload("Institute").Preload("Branch").Preload("Student").Raw("SELECT * FROM postponements").Scan(&Postponement).Find(&Postponement).Error; err != nil {
